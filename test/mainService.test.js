...
describe('mainService.getHomeData', () => {
    test('el contador de capítulos coincide dinámicamente con capitulosService (regresión: no debe volver a quedar hardcodeado)', () => {
        const data = mainService.getHomeData();
        expect(data.stats.capitulos).toBe(String(capitulosService.getAllCapitulos().length));
    });

    test('las features de "LA EXPERIENCIA" ya no mencionan Jet Skis ni "LUJO/VIP" (contenido retirado)', () => {
        const data = mainService.getHomeData();
    ...
    test('devuelve exactamente 3 features, cada una con icon/title/description', () => {
        const data = mainService.getHomeData();
